...
| StyleBI Docker + assistant 源码 | 待验证 | **可通（有条件）** | 直连可用前提：assistant 证书 SAN 包含 `host.docker.internal`/`host.containers.internal`，并使用 `STYLEBI_INTERNAL_URL=http://host.docker.internal:8080` 与 `chat.app.server.url=https://host.docker.internal:3001`；首次通常需浏览器手动证书授权 |
...
### 补充：StyleBI Docker + assistant 源码 的直连配置示例
...
在 **StyleBI 使用 Docker 启动**、**assistant 使用源码启动** 的组合下，可以采用直连模式。

- `server` 的 `.env`：

```env
STYLEBI_INTERNAL_URL=http://host.docker.internal:8080
```

- StyleBI 的 `sree.properties`：

```properties
ai.assistant.visible=true
chat.app.server.url=https://host.docker.internal:3001
```

> `chat.app.server.url` 指向 `https://host.docker.internal:3001` 时，通常需要先在浏览器中手动完成证书授权。

补充说明：

- `cert` 目录下已提供包含 `host.docker.internal` 的证书，可直接复制到 `server/cert` 目录下使用。
- 此类证书可用以下命令生成 `keystore.p12`：

```bash
openssl pkcs12 -export -in certificate.crt -inkey private.key -out keystore.p12 -name certificate
```
