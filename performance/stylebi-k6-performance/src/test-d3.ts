...
export default function () {
   ...
   let inactiveUserCount = parseInt(__ENV.INACTIVECOUNT) || 0;
   let inactiveUser = false;

// 如果设置了不活跃用户数，按 VU 序号分配
   if(inactiveUserCount > 0) {
   let totalUsers = parseInt(__ENV.USERS) || 10;
   inactiveUser = __VU <= inactiveUserCount;
   }
   let res2 = ws.connect(url, params2, function (socket) {
      ...
      let batchStage = new BatchStage([
                  new LambdaStage(() => startTime = Date.now()),
                  new SendAndListenStage(applySelectionWithCounter, "AddVSObjectCommand"),
                  new SendAndListenStage(refreshChartAreas.bind(null, chartName), "SetChartAreasCommand"),
                  new SendAndListenStage(clearSelection.bind(null, selectionListName), "AddVSObjectCommand"),
                  new SendAndListenStage(refreshChartAreas.bind(null, chartName), "SetChartAreasCommand"),
                  new LambdaStage(() => {
      ...
      if(inactiveUser) {
         batchStage = new BatchStage([
                 new SendStage(sendTouchAssetEvent),
                 new PauseStage(25000)
               ], {durationMs: 3600000});
      }
