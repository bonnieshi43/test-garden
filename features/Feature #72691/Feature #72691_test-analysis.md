...
**(4) 兼容性风险：默认值改变预期**   📅**测试-结果**：Bug #74071需要重启
...
#### 功能验证（Functional）   📅**测试-结果** Bug #74071
...
### 场景 1：开启 script.cursor.top 后，重开脚本面板应展示顶部（核心验收）📅**测试-结果**正确
...
### 场景 2：首次打开（缓存未命中）也必须置顶（异步时序风险）📅**测试-结果**正确
...
### 场景 3：默认行为回归（script.cursor.top=false 时仍可稳定置底）📅**测试-结果**正确
...
### 场景 4：边界脚本内��（空脚本/单行/末尾换行）  📅**测试-结果**正确
...
### 场景 5：后端接口不可用/返回异常时的降级体验     📅**测试-结果**忽略场景
